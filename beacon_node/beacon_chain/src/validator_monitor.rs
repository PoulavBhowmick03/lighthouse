...
use types::{
    ...
    PublicKeyBytes, SignedAggregateAndProof, SignedContributionAndProof, SingleAttestation, Slot,
    ...
    fn process_unaggregated_attestations(&mut self, state: &BeaconState<E>, spec: &ChainSpec) {
        ...
        for slot in attested_slots {
            if let Some(unaggregated_attestation) = unaggregated_attestations.remove(&slot) {
                ...
                match get_attestation_participation_flag_indices(state, data, inclusion_delay, spec)
                {
                    Ok(flag_indices) => {
